...
public class DeSSA {
	 ...
	 * SSA variables into normal variables and phi nodes into standard
	 * assignments.
	 */
	public void compute(final MethodDecl mdecl) {
		for (BasicBlock blk : new DepthFirstSearchPreOrder(mdecl.cfg)) {
			if (blk.phis.isEmpty())
				continue; // no phis, no work to do

			/**
			 * Build up a dependency graph. The nodes are variables defined by
			 * phi statements. The edges represent a copy of the nodes incoming
			 * value, so if a -> b then the value of a is stored into b. Note
			 * that every node in the graph has in-degree at most 1, because it
			 * can only get a value from one location. Some nodes have in-degree
			 * 0 if the value comes from a constant or a variable not created by
			 * a phi statement in this block.
			 * 
			 * To serialize the phi nodes, we walk this graph, forming a
			 * spanning tree. The value is assigned from the parent in the
			 * spanning tree. If while walking the graph we detect a cycle, we
			 * introduce a temporary to save the original value of one of the
			 * nodes in the cycle. These temporaries are given a guaranteed
			 * unique name like "(de-ssa-0)".
			 */
			...
			class DepNode {
				...
				/**
				 * Originally same as {@code originalSym}. If we introduce a
				 * temp, points to the temp.
				 */
				...
				void walkAndStore(List<Ast> addToList, DepNode takeValueFrom) {
					if (onStack) {
						...
						tempSym = new VariableSymbol(String.format(
								"(de-ssa-%d)", maxTemp++), originalSym.type,
								Kind.LOCAL);
					...
					addToList.add(new Assign(Var.withSym(originalSym), Var
							.withSym(takeValueFrom.tempSym)));
				}
			}
			...
			for (int i = 0; i < blk.predecessors.size(); i++) {
				...
				// Add edges:
				// For any assignments to a symbol X whose RHS is not from a phi
				// node
				...
				// Note that a variable X may appear as the RHS of other
				// symbols,
				...
				for (Phi phi : blk.phis.values()) {
					...
					if (rhsNode == null) { // RHS is not a phi variable defined
											// in this block. No dependencies,
						Assign assign = new Assign(Var.withSym(phi.lhs),
								rhsExpr);
						assigns.add(assign);
					} else {
