...
public class SemanticFailure extends RuntimeException {
	...
	public enum Cause {
		...
		/**
		 * Caused by an assignment to either a final field, {@code this}, or
		 * some other kind of expression which cannot be assigned to. <b>Not</b>
		 * used for type errors in assignments, which fall under
		 * {@link #TYPE_ERROR}.
		 */
		NOT_ASSIGNABLE,
		...
		/**
		 * Two variables, fields, methods, or classes with the same name were
		...
		/**
		 * A variable or other identifier was used in a method body which has no
		 * corresponding declaration
		 */
		NO_SUCH_VARIABLE,

		/**
		 * A method with a return type is missing a return statement among one
		 * of its paths
		 */
		MISSING_RETURN,
		 ...
		 * <li>An invalid cast was detected (i.e., a cast to a type that is not
		 * a super- or sub-type of the original type).
		 * <li>The size of an array in a new expression was not an int (i.e.,
		 * new A[i] where i is not an int)
		 * </ul>
		 */
		TYPE_ERROR,
		...
		/**
		 * A class {@code Object} was defined. This class is implicitly defined
		 * and cannot be defined explicitly.
		 */
		OBJECT_CLASS_DEFINED,

		/**
		 * A type name was found for which no class declaration exists. This can
		...
		/**
		 * The parameters of an overridden method have different types from the
		 * base method, there is a different number of parameters, or the return
		 * value is different.
		 */
		INVALID_OVERRIDE,
