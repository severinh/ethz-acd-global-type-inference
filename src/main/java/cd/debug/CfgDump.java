...
public class CfgDump {
	...
	public static void toString(MethodDecl mdecl, String phase, File filename,
	...
	public static void toString(List<? extends ClassDecl> astRoots,
	...
	public static String toString(List<? extends ClassDecl> astRoots,
			boolean dumpDominators) {
		return new CfgDump().dump(astRoots, dumpDominators);
	}

	public static String toString(MethodDecl mdecl, boolean dumpDominators) {
	...
	private String dump(MethodDecl mdecl, boolean dumpDominators) {
		sb.setLength(0);
		append("digraph G {");
	...
	private String dump(List<? extends ClassDecl> astRoots,
			boolean dumpDominators) {
		sb.setLength(0);
		append("digraph G {");
		append("graph [ rankdir = \"LR\" ];");
		...
		for (BasicBlock blk : mdecl.cfg.allBlocks) {
			...
			if (!dumpDominators || true) {
				...
				for (Entry<VariableSymbol, Phi> entry : blk.phis.entrySet()) {
					blklbl.append("|");
					blklbl.append(entry.getKey().name).append(": ")
			...
			for (int idx = 0; idx < blk.successors.size(); idx++) {
				...
				append("%sBB%d -> %sBB%d%s;", m, blk.index, m, sblk.index,
						edgelbl);
			}

			if (dumpDominators && blk.dominatorTreeParent != null) {
				append("%sBB%d -> %sBB%d [color=\"red\" label=\"dom\"];", m,
						blk.index, m, blk.dominatorTreeParent.index);
			}
		}
	}
